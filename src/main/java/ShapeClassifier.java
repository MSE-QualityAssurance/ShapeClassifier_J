...
public class ShapeClassifier {
  ...
  /**
   * This method takes in a comma-separated string argument containing the
   * dimensions of a shape and 3 guess values, namely:
   * 1. Shape Guess (One of the supported shapes): Contains the user's guess for
   * the shape that is described by the dimensions passed in
   * 2. Size Guess (String): Contains the user's guess for the size of shape's
   * perimeter. More rules on this provided below
   * 3. Even/Odd Guess (boolean): Contains the user's guess for whether the
   * shape's perimeter is odd or even ("Yes" = Even)
   * String arg is of following format: <ShapeGuess,SizeGuess,EvenOddGuess,Params>
   * Note: Params is a comma separated string containing the length of each side
   * of the shape.
   *
   * Supported shapes: "Line", "Circle", "Ellipse", "Rectangle", "Square",
   * "Equilateral", "Isosceles", "Scalene"
   * Supported size: "Small", "Large"
   *
   * For example, for an equilateral triangle, the Params could be = 100,100,100
   * 
   * The method returns "Yes" if all 3 guesses are correct. For any incorrect
   * guesses, it returns a comma-separated string
   * indicating which guesses were incorrect.
   * -> If >=3 guesses are incorrect then the program prints an error message and
   * exits
   * -> If the shape guess is incorrect, then the program returns a message with a
   * suggestion for the shape guess
   * -> If the size or even/odd guesses are incorrect, then the program returns a
  ...
  public String evaluateGuess(String arg) {
    try {
      ...
      // Validation flags
      boolean isShapeGuessCorrect = shapeGuessResult.equals(shapeGuess);
      boolean isSizeGuessCorrect = calcValue > 100 && sizeGuess.equals("Large") ||
          calcValue < 10 && sizeGuess.equals("Small");
      boolean isEvenOddCorrect = (calcValue % 2 == 0 && evenOddGuess.equals("Yes")) ||
          (calcValue % 2 != 0 && evenOddGuess.equals("No"));
      ...
      StringBuilder response = new StringBuilder("No: ");
      if (!isShapeGuessCorrect) {
        badGuesses++;
        if (badGuesses >= 3) {
          System.out.println("ERROR: Bad guess limit Exceeded");
          System.exit(1);
        }
        response.append(makeSuggestion(parameters, shapeGuess, shapeGuessResult));
      } else {
        badGuesses = 0;
        if (!isSizeGuessCorrect)
          response.append("Wrong Size,");
        if (!isEvenOddCorrect)
          response.append("Wrong Even/Odd");
      }
      return response.toString().replaceAll(",$", ""); // Remove trailing comma
    } catch (Exception e) {
      return "No";
    }
  }
  ...
  private String makeSuggestion(Integer[] parameters, String shapeGuess, String guessResult) throws Exception {
    if (parameters.length == 1) return "Suggestion=Line";
    if (parameters.length == 4) return shapeGuess.equals("Square") ? "Suggestion=Rectangle" : "Suggestion=Square";
    if (parameters.length == 2) return parameters[0] != parameters[1] ? "Suggestion=Ellipse" : "Suggestion=Circle";
    if (parameters.length == 3) {
        if (guessResult.isEmpty()) return "Suggestion=Not A Triangle";
        if (parameters[0] == parameters[1] && parameters[1] == parameters[2]) return "Suggestion=Equilateral";
        if (parameters[0] == parameters[1] || parameters[0] == parameters[2] || parameters[1] == parameters[2]) return "Suggestion=Isosceles";
